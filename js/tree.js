...
var input = document.getElementById("myInput")
	var inp = document.querySelectorAll('[id="myTable"]');  
	var inp2 = document.querySelectorAll('[id="closed"]');  
         
    if(input.value !=""){
		for (var i = 0; i < inp.length; i++) {
		inp[i].setAttribute('id',"closed");
		}   
        } else {
			for (var i = 0; i < inp2.length; i++) {
		inp2[i].setAttribute('id',"myTable");
		}   
        }
}
/*
...
function expand(){
	var img = document.getElementById("expando");
	var eleLi = document.querySelectorAll('[role="treeitem"]');  
	//if (eleLi.getAttribute('aria-expanded') == 'false') {	
         for (var i = 0; i < eleLi.length; i++) {
		eleLi[i].setAttribute('aria-expanded', true);
		//this.updateVisibleTreeitems();
		}   
		img.setAttribute('onclick',"collapse()");
		img.setAttribute('class',"btnopen");
...
function collapse(){
	...
	//if (eleLi.getAttribute('aria-expanded') == 'false') {	
         for (var i = 0; i < eleLi.length; i++) {
		eleLi[i].setAttribute('aria-expanded', false);
		//this.updateVisibleTreeitems();
		}
		games.setAttribute('aria-expanded', true);
		img.setAttribute('onclick',"expand()");
		img.setAttribute('class',"btn");
		img.src= "icons/expand.png";
}

function toggleDark(){
if(document.getElementById('darkmodecss').disabled){
	...
	localStorage.setItem("darkmodesave", "false");
	document.getElementById("btndark").src = "icons/dark.png";
}
}